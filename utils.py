...
def predict_map2coord(output, threshold=0.5):
    pred_dmap_low = F.relu(output)
    pred_dmap_low = F.avg_pool2d(pred_dmap_low, 2) * 4

    # pred_dmap = F.avg_pool2d(output, 2) * 4
    # draw_txt(pred_dmap, os.path.join(save_txt_dir, f"{img_id_list[idx]}_{h}_{w}_{h1}_{w1}.txt"))
    ...
    # pred_max = (pred_max == 3 ** 2 // 2).reshape(1, 1, H, W).long()
    # draw_map(pred_max.float(), os.path.join(save_dir, f"{idx}_pred_max.png"))

    kernel3x3 = torch.ones(1, 1, 3, 3).float().cuda()
    pred_cnt = F.conv2d(pred_dmap_low, weight=kernel3x3, padding=1)
    pred_filter = (pred_cnt > threshold)
    # pred_filter = (pred_cnt > threshold).long()
    # draw_map(pred_filter.float(), os.path.join(save_dir, f"{idx}_pred_filter.png"))

    pred_filter = pred_filter * pred_max
    # draw_map(pred_filter.float(), os.path.join(save_dir, f"{idx}_pred_filter_after.png"))
    # pred_filter = pred_filter.long().detach().cpu().squeeze(0).squeeze(0)
    ...
    coord_h_pred = (coord_h_unflod * pred_coord_weight).sum(dim=1, keepdim=True).reshape(H, W).detach().cpu()
    coord_w_unflod = F.unfold(coord_w, kernel_size=3, padding=1)
    coord_w_pred = (coord_w_unflod * pred_coord_weight).sum(dim=1, keepdim=True).reshape(H, W).detach().cpu()

    # print("filter", pred_filter.shape)
